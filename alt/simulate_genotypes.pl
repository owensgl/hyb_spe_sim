...
while (<GENO>){
	...
	my $P2count = 0;
        ...
        foreach my $sample_ID (1..$samplecounter){
            my $genotype;
	    my $tmp_info = $sampleinfo{$sample_ID}{$chr}{0};
	    my @tmp_blocks = split(//,$tmp_info);
	    my $per_block_length = $c{$chr}/$#tmp_blocks;
            my $current_block = $cm/$per_block_length;
            foreach my $strand (0..1){
                my $info = $sampleinfo{$sample_ID}{$chr}{$strand};
                my @blocks = split(//,$info);
                my $current_type = $blocks[$current_block];
	        if ($current_type eq "0"){
        	    my $tmp_base = $P1alleles[rand @P1alleles];
                    $genotype .= $tmp_base;
	        }elsif ($current_type eq "1"){
